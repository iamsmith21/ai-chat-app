...
In this repo, you have been given a working AI chat application built with Next.js, the Vercel AI SDK, and Claude. The chat interface is complete and functional. 
Your task is to **implement two tools** that give the AI chat application new capabilities.
...
1. Copy `.env.example` to `.env` and add the Anthropic API key attached in the technical assessment email 
2. Run `npm install`
3. Run `npm run dev`
4. Open http://localhost:3000
5. Verify basic chat works (send a message, get a response)
6. Implement the tools (Task 1 & 2) in `lib/tools/weather.ts` and `lib/tools/analyze.ts`
...
This assessment is designed to take roughly **1-2 hours**. Focus on getting the tools working correctly with good error handling. Don't worry about adding extra features. Clean, working code is what we are looking for.
